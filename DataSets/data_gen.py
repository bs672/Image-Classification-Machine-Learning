import numpy as np
from csv_read import load_extracted_features, load_graph, load_seed, load_seed_matrix, load_extracted_features_PCA
...
def seed_generator(batch_size=128,input_shape=(1,1084)):
    g = load_seed_matrix()
    f = load_extracted_features_PCA(k=input_shape[0]*input_shape[1], onlyseeds=True)
    nums = f.shape[0]
    input1 = np.empty(
        (batch_size, ) + input_shape)
    input2 = np.empty(
